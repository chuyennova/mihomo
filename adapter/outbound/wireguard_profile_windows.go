...
var _ wireguardDevice = (*windowsWireGuardTunDevice)(nil)

func newWindowsNetworkProfileDevice(option WireGuardOption, localPrefixes []netip.Prefix, mtu uint32) (_ wireguardDevice, err error) {
	if mtu == 0 {
...
func (d *windowsWireGuardTunDevice) DialTCP(ctx context.Context, network string, source, destination netip.AddrPort) (net.Conn, error) {
	if source.Addr().IsValid() || source.Port() != 0 {
		return nil, fmt.Errorf("Windows WireGuard system stack does not support an explicit TCP source endpoint")
	}
	if !destination.IsValid() {
		return nil, fmt.Errorf("Windows WireGuard system stack requires a resolved TCP destination")
	}
	remoteIP := destination.Addr().Unmap()
	destination = netip.AddrPortFrom(remoteIP, destination.Port())
	...
	if remoteIP.Is4() {
		...
		return d.tcp4.DialContext(ctx, "tcp4", destination.String())
	}
	if d.tcp6 == nil {
		return nil, fmt.Errorf("IPv6 is not configured on %q", d.name)
	}
	return d.tcp6.DialContext(ctx, "tcp6", destination.String())
}

func (d *windowsWireGuardTunDevice) DialUDP(ctx context.Context, network string, source, destination netip.AddrPort) (net.Conn, error) {
	if source.Addr().IsValid() || source.Port() != 0 {
		return nil, fmt.Errorf("Windows WireGuard system stack does not support an explicit UDP source endpoint")
	}
	if !destination.IsValid() {
	...
	if remoteIP.Is4() {
		...
		return d.udp4.DialContext(ctx, "udp4", destination.String())
	}
	if d.udp6 == nil {
		return nil, fmt.Errorf("IPv6 is not configured on %q", d.name)
	}
	return d.udp6.DialContext(ctx, "udp6", destination.String())
}

func (d *windowsWireGuardTunDevice) ListenUDP(ctx context.Context, network string, local netip.AddrPort) (net.PacketConn, error) {
	if local.Port() != 0 {
		return nil, fmt.Errorf("Windows WireGuard system stack only supports an automatically allocated UDP source port")
	}
	if local.Addr().IsValid() && !local.Addr().IsUnspecified() {
		localAddr := local.Addr().Unmap()
		if localAddr != d.v4 && localAddr != d.v6 {
			return nil, fmt.Errorf("UDP source address %s is not configured on %q", localAddr, d.name)
		}
	}
	switch strings.ToLower(network) {
	case "udp4":
	...
	case "udp":
		return nil, fmt.Errorf("Windows WireGuard system stack requires udp4 or udp6 for packet listeners")
	default:
		return nil, net.UnknownNetworkError(network)
	}
}
